"""Scatter BeamNG Forest items from Tirol forest masks (Hochwald / Strauch).
...
# Shapes used for scatter — paths filled after vendor_fir_pack().
SHAPE_FILES = {
...
def _item_template(internal: str, shape_vfs: str, *, radius: float, collidable: bool) -> dict:
    return {
        ...
        "trunkBendScale": 0.45 if collidable else 0.2,
        "branchAmp": 1.0,
        "detailAmp": 0.25,
        "detailFreq": 1.0,
        "annotation": "TREE" if collidable else "BUSH",
    }
...
def vendor_fir_pack(user_level: Path, level_name: str) -> dict[str, dict]:
    ...
    Cross-level shape refs load the mesh/imposters, but the sibling
    ``main.materials.json`` from driver_training is not registered — close
    LODs then render as neon orange (missing material).
    """
    ...
    return {
        "autoroad_fir_small": _item_template(
            "autoroad_fir_small", f"{base}/{SHAPE_FILES['autoroad_fir_small']}", radius=1.5, collidable=True
        ),
        "autoroad_fir_large": _item_template(
            "autoroad_fir_large", f"{base}/{SHAPE_FILES['autoroad_fir_large']}", radius=2.0, collidable=True
        ),
        "autoroad_fir_bush": _item_template(
            "autoroad_fir_bush", f"{base}/{SHAPE_FILES['autoroad_fir_bush']}", radius=0.8, collidable=False
        ),
    }
...
def _yaw_matrix(yaw_rad: float) -> list[float]:
    ...
    # BeamNG forest rotationMatrix is row-major 3x3 (Z-up yaw).
...
def scatter(site: dict, cfg: dict) -> dict[str, list[dict]]:
    ...
    def try_place(kind: str, r: float, c: float, scale_lo: float, scale_hi: float) -> str:
        """Return 'ok' | 'skip' | 'full'."""
        nonlocal total
        if total >= max_items:
            return "full"
        ...
        if bx < 0 or by < 0 or bx > extent or by > extent:
            return "skip"
        if float(slope_at(bx, by)) > slope_max:
            return "skip"
        z = float(z_at(bx, by))
    ...
    for r, c in _candidate_cells(high, float(cfg["spacing_high_m"]), mpp, rng, jitter):
        kind = "autoroad_fir_large" if rng.random() < 0.35 else "autoroad_fir_small"
        status = try_place(kind, r, c, float(s_hi[0]), float(s_hi[1]))
        if status == "full":
            break

    for r, c in _candidate_cells(scrub, float(cfg["spacing_scrub_m"]), mpp, rng, jitter):
        status = try_place("autoroad_fir_bush", r, c, float(s_sc[0]), float(s_sc[1]))
        if status == "full":
            break

    print(
        f"Forest scatter: high={len(placements['autoroad_fir_large'])+len(placements['autoroad_fir_small'])} "
        f"scrub={len(placements['autoroad_fir_bush'])} total={total} "
        f"(cap={max_items})"
    )
    return placements
...
def ensure_forest_object(user_level: Path) -> None:
    ...
    setup_beamng_level strips GroundCover and used to drop Forest too — put it back.
    """
    ...
    # Ensure SimGroup vegetation is registered
...
def write_forest4(user_level: Path, proc: Path, placements: dict[str, list[dict]]) -> None:
    ...
    # Remove previous autoroad forest files only
    ...
    for kind, rows in placements.items():
        ...
        # mirror under processed for inspection
...
def main() -> None:
    ...
    proc = processed_dir(site)
    # masks must exist
    if not (proc / "mask_forest_high.png").is_file():
    ...
    (proc / "forest_scatter_summary.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
    ...
    items = vendor_fir_pack(user_level, level_name)
